...
#remove ki009 and dr033 - they have a developmental delay
pdr <- pdr %>% 
  filter(subid != "ki009" & subid != "dr033")
...
look_mod <- lmer(prop_looking ~ partner + (1|subid) + (1|vid), data = vid_looking)
summary(look_mod)
...
```
# Write data file for manuscript.
```{r write data file}
write_csv(bounds_1s, "cleaned_pdr_data.csv")
write_csv(vid_looking, "cleaned_looking_data.csv")
```